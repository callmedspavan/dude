import React, {useState} from 'react';
import {Image, StyleSheet, View} from 'react-native';
import * as yup from 'yup';
import {
  widthPercentageToDP as wp,
  heightPercentageToDP as hp,
} from 'react-native-responsive-screen';

import routeName from '../Config/navigationRouteNames';

import AppLinkText from '../Components/AppLinkText';
import AppScreen from '../Components/AppScreen';
import AppText from '../Components/AppText';
import AppForm from '../Components/formComponents/AppForm';
import AppFormField from '../Components/formComponents/AppFormField';
import AppFormSubmitButton from '../Components/formComponents/AppFormSubmitButton';
import AppCheckbox from '../Components/AppCheckbox';
import defaultStyles from '../Config/defaultStyles';
import externalLinks from '../Config/externalLinks';

const MobileNumberEnterScreen = ({navigation}) => {
  const [whatsappUpdates, setWhatsappUpdates] = useState(0);
  const Schema = yup.object().shape({
    phoneNumber: yup
      .string()
      .required()
      .min(10)
      .matches(/^[6-9]{1}[0-9]{9}$/),
  });

  const handleSubmit = (values, whatsappUpdates) => {
    // calling Backend
    navigation.navigate(routeName.MOBLE_OTP_ENTER_SCREEN, {
      phoneNumber: values.phoneNumber,
    });
  };

  return (
    <AppScreen>
      <View style={styles.screen}>
        <Image
          source={require('../Assets/logo.png')}
          resizeMode={'contain'}
          style={styles.logo}
        />
        <AppText style={styles.heading}>Enter your mobile number</AppText>
        <View style={styles.headingUnder} />

        <AppForm
          initialValues={{phoneNumber: ''}}
          validationSchema={Schema}
          onSubmit={values => handleSubmit(values, whatsappUpdates)}>
          <AppFormField
            name="phoneNumber"
            startText="+91"
            placeholder="10 digit mobile number"
            preCheck={true}
            keyboardType="number-pad"
            maxLength={10}
          />
          <AppFormSubmitButton style={styles.button} title="Send OTP" />
        </AppForm>

        <View style={styles.whatsappRow}>
          <AppCheckbox
            checked={whatsappUpdates}
            onPress={() => setWhatsappUpdates(whatsappUpdates ? 0 : 1)}
          />
          <AppText style={styles.whatsappText}>
            Receive updates on Whatsapp
          </AppText>
          <Image
            style={styles.whatsappLogo}
            source={require('../Assets/WhatsappLogo.webp')}
            resizeMode="contain"
          />
        </View>

        <View style={styles.termsRow}>
          <AppText style={styles.termsText}>
            By continuing, you are agreeing to our
          </AppText>
          <AppLinkText
            style={styles.termsLink}
            title=" T&C "
            url={externalLinks.termsAndConditions}
          />
          <AppText style={styles.termsText}>and </AppText>
          <AppLinkText
            style={styles.termsLink}
            title="Privacy policy "
            url={externalLinks.policy}
          />
        </View>
      </View>
    </AppScreen>
  );
};

const styles = StyleSheet.create({
  screen: {
    flex: 1,
    padding: wp(7),
  },
  logo: {
    height: wp(20),
    marginTop: hp(2),
    width: wp(45),
  },
  heading: {
    color: defaultStyles.colors.black,
    fontWeight: '500',
    letterSpacing: 1.5,
    marginTop: hp(3.5),
  },
  headingUnder: {
    height: hp(0.8),
    width: wp(9),
    backgroundColor: defaultStyles.colors.primary,
    marginTop: hp(1),
    borderRadius: 4,
    marginBottom: hp(4),
  },
  button: {
    bottom: hp(2),
    left: wp(7),
    position: 'absolute',
  },
  whatsappRow: {
    flexDirection: 'row',
    marginTop: hp(3),
    alignItems: 'center',
  },
  whatsappText: {
    fontSize: 13,
    marginLeft: wp(3.5),
    color: defaultStyles.colors.medium,
  },
  whatsappLogo: {
    width: wp(5.5),
    height: wp(5.5),
    marginLeft: wp(3),
  },
  termsRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: hp(4),
    width: wp(80),
    flexWrap: 'wrap',
  },
  termsText: {
    fontSize: 12.5,
    color: defaultStyles.colors.medium,
  },
  termsLink: {
    fontSize: 12.5,
  },
});

export default MobileNumberEnterScreen;
